...
class Spider(object):
	...
	def login(self):
		try:
			res = self.session.post(self.loginUrl, data = self.data, headers = self.headers, proxies = self.proxies)
			return res.cookies
		except Exception, e:
	...
	def getIds(self):
		...
		if cookies:
			try:
				response = self.session.get(self.destUrl, cookies = cookies, headers = self.headers)
			except Exception, e:
			...
			while i <= pages:
				url = formatUrl % i
				try:
					response = self.session.get(url, cookies = cookies, headers = self.headers)
				except Exception, e:
					print "getIds() get error, ", e
				content = response.content
